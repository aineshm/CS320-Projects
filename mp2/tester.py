...
def compare_float(expected, actual, config={}):
    if math.isnan(expected) and math.isnan(actual):
        return True
    tolerance = float(config.get("tolerance", 0.00001))
...
def main():
    ...
    #check if additional points have been added to loan tester
    original_count = 16
    # change original to match number in the original module tester script
    with open(r"module_tester.py","r") as modtext:
        lines = modtext.read().split("\n")
    count = 0
    loanstr = "loans_points +="
    for row in lines:
        if row.find(loanstr) != -1:
                count += 1
    if count == original_count:
        print("Additional points/test cases were not added to module_tester.")

...
def app_req(
    ...
    if expect_str:
        ...
        if not isinstance(body, str):
            raise TypeError(
                f"Expected request {path} to return a string encoded as UTF-8"
            )
...
def edgar_utils():
    ...
    return module_result["score"] * 0.2


def compare_dicts(dict1, dict2):
    # new dict comparison
    # no longer evals through exact string comparison
    try:
    ...
    for link in svg_path_ll:
        ...
        if status == "200 OK":
            ...
            if (
                ...
                or headers.get("Content-type") == "image/svg+xml"
            ):
                svg_points += 2
            else:
                print(headers)
